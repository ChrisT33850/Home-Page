...
export default class AgentsOnline extends LightningElement {
    ...
    getAgentPresence(agentId) {
        return this.agentsPresence[agentId] || {
            statusId: null,
            statusName: 'Unknown',
            statusColor: '#888'
        };
    ...
    get agentStats() {
        ...
        this.agentsInQueue.forEach(agent => {
            const presence = this.getAgentPresence(agent.userId);
            const status = presence.statusName || 'Unknown';
            
            if (status.includes('Available')) {
                available++;
            } else if (status.includes('Busy') || status.includes('In Call')) {
                busy++;
